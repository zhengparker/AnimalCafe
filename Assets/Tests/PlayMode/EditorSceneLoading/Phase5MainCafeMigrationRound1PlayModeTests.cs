...
namespace AnimalCafe.Tests.PlayMode
{
    ...
    {
        [UnityTearDown]
        public IEnumerator UnloadMainCafeBeforeInputFixtureRestoresGlobalInput()
        {
            yield return UnloadLoadedMainCafe();
        }
        ...
        private static IEnumerator UnloadLoadedMainCafe()
        {
            var loaded = Enumerable.Range(0, SceneManager.sceneCount)
                .Select(SceneManager.GetSceneAt)
                .Where(scene => scene.isLoaded && scene.name == "MainCafe")
                .ToArray();
            if (loaded.Length == 0) yield break;

            var cleanup = SceneManager.CreateScene("Phase5MainCafeMigrationRound1TestCleanup");

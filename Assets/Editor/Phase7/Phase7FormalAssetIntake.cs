...
namespace AnimalCafe.EditorTools.Phase7
{
    public static class Phase7FormalAssetIntake
    {
        ...
        // Save only the generated materials, definitions and catalogues owned by this builder.
        // 仅保存本工具生成的资源，保留其他资源的未保存编辑。
        public static void Build()
        {
            ...
            WriteManifest();AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
        }
        ...
        private static void ApplyTechnicalMaterials(GameObject visual,Item item)
        {
            ...
            if(item.Id=="wall-decor.shiba-painting.01")
            {
                ...
                var material=EnsureMaterial(Phase7AssetPaths.MaterialFolder+"/M_WallDecor_ShibaPortrait_01.mat",shader);material.SetTexture("_BaseMap",AssetDatabase.LoadAssetAtPath<Texture2D>(texturePath));material.SetColor("_BaseColor",Color.white);ConfigureAlphaBlend(material);EditorUtility.SetDirty(material);AssetDatabase.SaveAssetIfDirty(material);
            ...
            {
                var glass=EnsureMaterial(Phase7AssetPaths.MaterialFolder+"/M_Window_TallGlass_Transparent.mat",shader);glass.name="M_Window_TallGlass_Transparent";glass.SetColor("_BaseColor",new Color(.72f,.9f,.96f,.28f));glass.SetFloat("_Surface",1f);glass.SetFloat("_Blend",0f);glass.SetInt("_SrcBlend",(int)UnityEngine.Rendering.BlendMode.SrcAlpha);glass.SetInt("_DstBlend",(int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);glass.SetInt("_ZWrite",0);glass.EnableKeyword("_SURFACE_TYPE_TRANSPARENT");glass.renderQueue=3000;EditorUtility.SetDirty(glass);AssetDatabase.SaveAssetIfDirty(glass);
        ...
        private static void Publish(string path,WallMountedCatalogueKind kind,IEnumerable<WallMountedDefinitionAsset> defs){var c=EnsureAsset<WallMountedCatalogueAsset>(path);Set(c,"kind",(int)kind);SetArray(c,"entries",defs.Cast<UnityEngine.Object>().ToArray());AssetDatabase.SaveAssetIfDirty(c);}
